...
if __name__ == "__main__":
    ...
    try:
        ...
        # audio_generator_process = Process(target=run_main, args=(args, logger, config, text_queue, audio_name_queue, "audio_generator"))
        # audio_editor_process = Process(target=run_main, args=(args, logger, config, audio_name_queue, edited_audio_queue, "audio_editor"))
        speaker_process = Process(target=run_main, args=(args, logger, config, text_queue, None, "speaker"))

        speech_recognizer_process.start()
        transcriptor_process.start()
        # translator_process.start()
        # audio_generator_process.start()
        # audio_editor_process.start()
